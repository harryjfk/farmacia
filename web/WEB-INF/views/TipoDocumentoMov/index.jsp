...
<script>
    ...
    $('#btnExcel').click(function(e) {
        ...
        window.location = getUrlFromDatatables('<c:url value="/TipoDocumentoMov/excel" />', dataTable);
    });
    ...
    $('#btnPDF').click(function(e) {
        ...
        var dataTable = $(tblTipoDocumentoMov).dataTable();
        window.location = getUrlFromDatatables('<c:url value="/TipoDocumentoMov/pdf" />', dataTable);
    });
    ...
    <c:forEach var="opcionSubmenu" items="${opcionesSubmenu}">
        <c:if test="${opcionSubmenu.appOpcion == 'modificar'}">
            editHTML += '<a href="#" class="separator-icon-td" onclick="obtenerTipoDocumentoMov(' + data + ', event, this)" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-pencil"></i></a>';
        </c:if>
        <c:if test="${opcionSubmenu.appOpcion == 'estado'}">
            stateHTML += '<a href="#" class="separator-icon-td" onclick="cambiarEstadoTipoDocumentoMov(' + data + ', event, this)" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-refresh"></i></a>';
        </c:if>
        <c:if test="${opcionSubmenu.appOpcion == 'eliminar'}">
            deleteHTML += '<a href="#" class="separator-icon-td" onclick="eliminarTipoDocumentoMov(' + data + ', event, this)" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-remove"></i></a>';
        </c:if>
    </c:forEach>
    ...
    function obtenerTipoDocumentoMov(id, e, element) {
        ...
        modalTipoDocumentoMov.find('.modal-header .modal-title').html($(element).attr('title'));
        cleanform('#modalTipoDocumentoMov');
        $('#chkActivo').removeAttr('disabled');

        $.ajax({
            url: '<c:url value="/TipoDocumentoMov/tipoDocumentoMovJSON"/>/' + id,
            type: 'GET',
            dataType: 'json',
    ...
    $('#btnAgregar').click(function(e) {
        ...
        modalTipoDocumentoMov.find('.modal-header .modal-title').html($(this).attr('title'));
        cleanform('#modalTipoDocumentoMov');
    ...
    function cambiarEstadoTipoDocumentoMov(id, e, element) {
        ...
        smokeConfirm('¿Está seguro que desea cambiar de estado el tipo de documento de movimiento ' + tipoDocumentoMovTexto + '?', function(e) {
            if (e) {
                $.ajax({
    ...
    function eliminarTipoDocumentoMov(id, e, element) {
        ...
        smokeConfirm('¿Está seguro que desea eliminar el tipo de documento de movimiento ' + tipoDocumentoMovTexto + '?', function(e) {
            if (e) {
                $.ajax({
