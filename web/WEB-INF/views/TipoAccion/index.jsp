...
<script>
    ...
    $('#btnExcel').click(function(e) {
        ...
        window.location = getUrlFromDatatables('<c:url value="/TipoAccion/excel" />', dataTable);
    });
    ...
    $('#btnPDF').click(function(e) {
        ...
        var dataTable = $(tblTipoAccion).dataTable();
        window.location = getUrlFromDatatables('<c:url value="/TipoAccion/pdf" />', dataTable);
    });
    ...
    function listarTiposAcciones() {
        ...
        $(tblTipoAccion).dataTable({
            "sAjaxSource": "<c:url value="/TipoAccion/tiposAccionesJSON"/>",
            "bServerSide": true,
            "bProcessing": true,
    ...
    <c:forEach var="opcionSubmenu" items="${opcionesSubmenu}">
        <c:if test="${opcionSubmenu.appOpcion == 'modificar'}">
                        editHTML += '<a href="#" class="separator-icon-td" onclick="obtenerTipoAccion(' + data + ', event, this)" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-pencil"></i></a>';
        </c:if>
        <c:if test="${opcionSubmenu.appOpcion == 'estado'}">
                        stateHTML += '<a href="#" class="separator-icon-td" onclick="cambiarEstadoTipoAccion(' + data + ', event, this)" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-refresh"></i></a>';
        </c:if>
        <c:if test="${opcionSubmenu.appOpcion == 'eliminar'}">
                        deleteHTML += '<a href="#" class="separator-icon-td" onclick="eliminarTipoAccion(' + data + ', event, this)" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-remove"></i></a>';
        </c:if>
    </c:forEach>
    ...
    function obtenerTipoAccion(id, e, element) {
        ...
        modalTipoAccion.find('form').attr('action', '<c:url value="/TipoAccion/modificar" />');
        modalTipoAccion.find('.modal-header .modal-title').html($(element).attr('title'));
        cleanform('#modalTipoAccion');
        $('#chkActivo').removeAttr('disabled');

        $.ajax({
            url: '<c:url value="/TipoAccion/tipoAccionJSON" />/' + id,
            type: 'GET',
            dataType: 'json',
    ...
    $('#btnAgregar').click(function(e) {
        ...
        var modalTipoAccion = $('#modalTipoAccion');
        modalTipoAccion.find('form').attr('action', '<c:url value="/TipoAccion/registrar" />');
        modalTipoAccion.find('.modal-header .modal-title').html($(this).attr('title'));
        cleanform('#modalTipoAccion');
        $('#idTipoAccion').val('0');
        $('#activo').val('1');
    ...
    function cambiarEstadoTipoAccion(id, e, element) {
        ...
        smokeConfirm('¿Está seguro que desea cambiar de estado el tipo de acción ' + tipoAccionTexto + '?', function(e) {
            if (e) {
                $.ajax({
                    url: '<c:url value="/TipoAccion/estado" />/' + id,
                    type: 'POST',
                    dataType: 'json',
    ...
    function eliminarTipoAccion(id, e, element) {
        ...
        smokeConfirm('¿Está seguro que desea eliminar el tipo de acción ' + tipoAccionTexto + '?', function(e) {
            if (e) {
                $.ajax({
                    url: '<c:url value="/TipoAccion/eliminar" />/' + id,
                    type: 'POST',
                    dataType: 'json',
                    success: function(dataResponse, status, metaData) {
                        var f = function() {
