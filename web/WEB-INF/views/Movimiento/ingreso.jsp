...
<div class="row">
    <div class="col-sm-12 col-md-12">
        <table id="tblIngreso" class="table table-bordered table-striped table-condensed">
            <thead>
                <tr>
...
<script>
    var tblIngreso = document.getElementById('tblIngreso');
    ...
    $('#btnExcel').click(function(e) {
        ...
        var dataTable = $(tblIngreso).dataTable();
        window.location = getUrlFromDatatables('<c:url value="/NotaIngreso/excel" />', dataTable);
    });
    ...
    $('#btnPDF').click(function(e) {
        ...
        var dataTable = $(tblIngreso).dataTable();
        window.location = getUrlFromDatatables('<c:url value="/NotaIngreso/pdf" />', dataTable);
    });
    
    $('#btnAgregar').click(function(e) {
        e.preventDefault();

        window.location = '<c:url value="/NotaIngreso/registrar" />';
    });
    
    $('#btnBuscar').click(function(e) {
        e.preventDefault();
        var dataResponse = {mensajesRepuesta:[]}; 
    ...
    function listarIngresos() {
        ...
        if ($.fn.DataTable.fnIsDataTable(tblIngreso)) {
        ...
        $(tblIngreso).dataTable({
            ...
            "sAjaxSource": '<c:url value="/NotaIngreso/ingresosJSON" />',
            "bServerSide": true,
            "bProcessing": true,
            ...
            "aoColumns": [
                ...
                {mData: "almacenOrigen", sWidth: "15%", "bSortable": false},
                {mData: "almacenDestino", sWidth: "15%", "bSortable": false},
                {mData: "tipoDocumento", sWidth: "11%", "bSortable": false},
                {mData: "numeroDocumento", sWidth: "10%", "bSortable": false},
                {mData: "documentoOrigen", sWidth: "11%", "bSortable": false},
                {mData: "numeroDocumentoOrigen", sWidth: "10%", "bSortable": false},
                {mData: "idMovimiento", sWidth: "10%", "bSortable": false, "mRender": function(data, type, row) {
                    ...
                    <c:forEach var="opcionSubmenu" items="${opcionesSubmenu}">
                        <c:if test="${opcionSubmenu.appOpcion == 'modificar'}">
                            editHTML += '<a href="<c:url value="/NotaIngreso/modificar" />/' + data + '" class="separator-icon-td" title="${opcionSubmenu.nombreOpcion}"><i class="splashy-pencil"></i></a>';
                        </c:if>
                        ...
                        <c:if test="${opcionSubmenu.appOpcion == 'imprimir'}">
                            printHTML += '<a href="#" class="separator-icon-td" title="${opcionSubmenu.nombreOpcion}" onclick="printIngreso(' + data + ', event)"><i class="splashy-download"></i></a>';
                        </c:if>
                    </c:forEach>
    ...
    function printIngreso(idIngreso, e){
        e.preventDefault();
        window.location = '<c:url value="/NotaIngreso/imprimir" />/' + idIngreso;
    }
</script>