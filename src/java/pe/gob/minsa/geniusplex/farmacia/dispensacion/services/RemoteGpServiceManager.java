 ...
 * @param <T>
 */
public abstract class RemoteGpServiceManager<T extends RemoteEntity> implements GServiceManager<T> {

    protected Class<T> entityClass;
    protected EntityManager em;

    public RemoteGpServiceManager(Class<T> entityClass) {
    ...
    public boolean cambiarEstado(Object id) {
        throw new UnsupportedOperationException("Aqui no sirve cambiar estado, lo llamaste por gusto GGGG."); //To change body of generated methods, choose Tools | Templates.
    }
    ...
    /**
     * Sincroniza la entidad t con la base de datos (trae los datos reales de
     * dicha entidad) Levanta una exepcion si la entidad no existe.
     *
     ...
     * API to build a dynamic query. If the field you want to apply the filter
     * on is a java.util.Date, its value has to be the concatenation of the
     * date, a semi-colon (:), and the date format. To apply the filter on
     * relationships you have to use period (.), example lote.description to
     * filter by the product's lote description.
     *
                ...
                if (field.getType().getName().equalsIgnoreCase("java.lang.string")) {
                    ...
                    if (isDate) {//si el campo es de tipo fecha, parsear el value a java.util.Date (no se como va a funcionar con java.sql.Date)
                        ...
                        if (tmpValue.length > 1) {
                            ...
                            ((Calendar)value).setTimeInMillis(Long.parseLong(tmpValue[0]));
                        }
                    }
